...
public class SetTaskConverter implements AttributeConverter<EnumSet<Task>, String> {

    // DB에서 사용할 Type으로 Entity Type을 변환하는 로직 구현
    @Override
    public String convertToDatabaseColumn(EnumSet<Task> attribute) {
    ...
    @Override
    public EnumSet<Task> convertToEntityAttribute(String dbData) {
        // DB에서 읽어온 값이 null이거나 공백이거나 Task.DESIGNER 형태로 읽어올 경우 제외
        if(dbData == null || dbData == "" || dbData.contains(".")) return EnumSet.noneOf(Task.class);
        // 최초 빈 Collection 생성
        EnumSet<Task> attribute = EnumSet.noneOf(Task.class);
        ...
        Arrays.stream(dbDataArray).forEach(e -> attribute.add(Task.valueOf(e)));
        return attribute;
    }
}