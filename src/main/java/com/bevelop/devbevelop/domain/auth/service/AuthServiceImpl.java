...
//        return responseService.getFailResult(400,"Fail to Sign up");
	...
	public ResponseEntity<JSONObject> login(UserLogInDto userLoginDto) {
		...
		try {
			...
			User userDetail = userRepository.findByEmail(userLoginDto.getEmail())
...
//    }
	...
	public CommonResult logout(UserLogOutDto userLogOutDto) {
		...
		Authentication authentication = jwtTokenProvider
				.getAuthenticationByAccessToken(userLogOutDto.getAccess_token());
		...
		redisTemplate.opsForValue().set(userLogOutDto.getAccess_token(), "logout", expiration, TimeUnit.MILLISECONDS);
